    ...
    for k in range(S_list.shape[0]):
        ...
        for m in range(n_replicate):
            ...
            if m == 0:
                ...
                plot_A_sub(A[order_row_opt, :][:, order_col_opt], 'Reordered input\nmatrix, ' +
                           r'$t=' + str(k + 1) + r'$' + '\n(DeepTMR)',
                ...
                plot_A_sub(A[order_row_opt_pca, :][:, order_col_opt_pca], 'Reordered input\nmatrix, ' +
                           r'$t=' + str(k + 1) + r'$' + '\n(SVD-Angle)',
                           'synthetic4_input_sort_svd_angle', clr_matrix, n_row_fig, n_col_fig, k + 1, fig_A_pca.number)
                plot_A_sub(A[order_row_opt_svd, :][:, order_col_opt_svd], 'Reordered input\nmatrix, ' +
                           r'$t=' + str(k + 1) + r'$' + '\n(SVD-Rank-One)',
                           'synthetic4_input_sort_svd_rank_one', clr_matrix, n_row_fig, n_col_fig, k + 1, fig_A_svd.number)
                plot_A_sub(A[order_row_opt_mds, :][:, order_col_opt_mds], 'Reordered input\nmatrix, ' +
                           r'$t=' + str(k + 1) + r'$' + '\n(MDS)',
